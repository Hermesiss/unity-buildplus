...
namespace Trismegistus.BuildPlus
{
    public sealed class BuildPlusEditor : EditorWindow
    {
        ...
        {
            var package = GetPackageInfo("trismegistus.unity.buildplus");

            Debug.Log($"Installed version : {package.version}");
        ...
        void OnGUI()
        {
                ...
                position.height - padding * 2f));
            {
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    {
                        if (GUILayout.Button("Report bug or leave a feedback", EditorStyles.miniButton))
                        {
                            Application.OpenURL("www.github.com/Hermesiss/unity-buildplus/issues");
                        }
                    }
                }
                ...
                var buildEnabled = IsTargetSupported(EditorUserBuildSettings.activeBuildTarget);
                ...
                EditorGUILayout.BeginVertical();
                {
                    ...
                    if (buildEnabled)
                        ...
                        EditorGUILayout.HelpBox(GetBuildPath(), MessageType.Error);
                }
        ...
        private bool IsTargetSupported(BuildTarget target)
        {
            switch (target)
            {
